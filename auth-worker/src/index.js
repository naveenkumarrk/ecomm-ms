...
async function hashPassword(password) {
	...
	const bits = await crypto.subtle.deriveBits({ name: 'PBKDF2', hash: 'SHA-256', salt, iterations: 20000 }, key, 256);
...
router.post('/auth/signup', async (req, env) => {
	...
	if (!body.email || !body.password || !body.name) return json({ error: 'missing_fields' }, 400);
...
router.post('/auth/admin/promote', async (req, env) => {
	...
	const user = await dbGet(env, body.userId ? 'SELECT * FROM users WHERE userId=?' : 'SELECT * FROM users WHERE email=?', [identifier]);

	if (!user) return json({ error: 'user_not_found' }, 404);
	if (user.role === 'admin') return json({ error: 'already_admin' }, 400);

	await dbRun(env, 'UPDATE users SET role=?, updated_at=? WHERE userId=?', ['admin', epoch(), user.userId]);
...
router.post('/auth/login', async (req, env) => {
	...
	if (!body.email || !body.password) return json({ error: 'missing_fields' }, 400);
	...
	await dbRun(env, 'INSERT INTO sessions (id,userId,created_at,expires_at,revoked) VALUES (?,?,?,?,0)', [sid, user.userId, now, exp]);
...
router.post('/auth/addresses', async (req, env) => {
	...
	const body = await req.json();
	if (!body.line1 || !body.city || !body.postal) return json({ error: 'missing_fields' }, 400);
	...
	await dbRun(env, 'UPDATE users SET data=?, updated_at=? WHERE userId=?', [JSON.stringify(d), epoch(), u.sub]);
...
router.put('/auth/addresses/:id', async (req, env) => {
	...
	await dbRun(env, 'UPDATE users SET data=?,updated_at=? WHERE userId=?', [JSON.stringify(data), epoch(), u.sub]);
...
router.delete('/auth/addresses/:id', async (req, env) => {
	...
	if (newList.length === data.addresses.length) return json({ error: 'address_not_found' }, 404);
	...
	async fetch(req, env, ctx) {
		try {
			...
			if (span) {
				...
				span.setAttribute('service.name', env.SERVICE_NAME || 'auth-worker');
			}
...
const config = (env, _trigger) => {
	...
	return {
		...
		service: {
			name: env.SERVICE_NAME || 'auth-worker',
		},
